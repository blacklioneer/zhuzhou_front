...
class TimelineChart extends React.Component {
  render() {
    ...
    const data1 = Array.isArray(sourceData) ? sourceData : [{ x: 0, y1: 0, y2: 0 }];
    data1.sort((a, b) => a.x - b.x);

    let max;
    if (data1[0] && data1[0].y1 && data1[0].y2) {
      max = Math.max(
        [...data1].sort((a, b) => b.y1 - a.y1)[0].y1,
        [...data1].sort((a, b) => b.y2 - a.y2)[0].y2
      );
    }
