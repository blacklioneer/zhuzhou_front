...
class TimelineChart extends React.Component {
  render() {
    ...
    const data = Array.isArray(sourceData) ? sourceData : [{ x: 0, y1: 0, y2: 0 ,y3: 0,/* y4: 0, y5: 0, y6: 0, */}];

    data.sort((a, b) => a.x - b.x);

    let max;
    if (data[0] && data[0].y1 && data[0].y2 && data[0].y3 /* && data[0].y4 && data[0].y5 && data[0].y6 */ ) {
      max = Math.max(
      ...
      <div className={styles.timelineChart} style={{ height: height + 30 }}>
        <div>
          ...
          <Chart height={height} padding={padding} data={dv} scale={cols} forceFit>
            ...
            <Geom type='point' position="x*value" size={2} shape='point' color='#fff'  />
          </Chart>
